...
import {
  writeFileSync, appendFileSync, readdirSync, unlinkSync, existsSync, readFileSync, mkdirSync,
} from 'fs';
...
const spriteBuild = (data) => {
  ...
  src(allIcons)
    ...
    .on('data', (file) => {
      appendFileSync(spriteFilePath, getSptireStyleTemplate(data.name, file.stem, spriteType));
      return file;
    })
...
const svgSpriteBuild = (done) => {
  ...
  if (!existsSync(config.svgSprites.spritesFolder)) {
    ...
    readdirSync(config.svgSprites.spritesFolder)
      .forEach((file) => {
...
const svgSpriteWatch = () => watch(
  config.svgSprites.watch,
  series(serverReload),
